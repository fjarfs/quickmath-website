...
const PromoTnc = ({ checkMatchCourses }) => {
  const [toggle, setToggle] = useState(false);
  const { title, desc, descTwo, tncList } = checkMatchCourses;
          ...
          <div className="row g-5 mb--30">
            <div className="col-lg-6">
              <ul className="rbt-list-style-1">
                {tncList.slice(0, 4).map((item, innerIndex) => (
                  <li key={innerIndex}>
            ...
            <div className="col-lg-6">
              <ul className="rbt-list-style-1">
                {tncList.slice(4, 8).map((item, innerIndex) => (
                  <li key={innerIndex}>
                    <i className="feather-check"></i>
                    {item.listItem}
                  </li>
